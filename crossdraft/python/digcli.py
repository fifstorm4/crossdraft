...
def _outdir(spec, args):
    d = args.outdir or os.path.join(HERE, "build", spec["name"])
...
def cmd_bound(args):
    ...
    differential_bound(cipher, [int(w) for w in args.weights.split(",")],
                       key_bits=spec["key_bits"],
                       block_bits=spec["block_bits"], solver=args.solver)
    return 0
...
def cmd_relatedkey(args):
    ...
    r = related_key(cipher, key_bits=spec["key_bits"],
                    block_bits=spec["block_bits"], solver=args.solver)
    print()
    ks = key_schedule_report(cipher, key_bits=spec["key_bits"],
                             samples=args.samples)
    print(f"  affine in the key: {ks['linear_in_key']}")
    print(f"  {ks['note']}")
    if args.json:
        with open(args.json, "w") as fh:
...
def cmd_random(args):
    ...
    print()
    d = diffusion(cipher, trials_per_bit=max(4, args.samples // 25))
...
def main():
    ...
    p.add_argument("cipher")
    p.add_argument("--rounds", type=int)
    p.add_argument("--weights", default="8,12,16,20")
    p.add_argument("--solver", default="KISSAT_EXT")
    ...
    return {"list": cmd_list, "build": cmd_build, "verify": cmd_verify,
