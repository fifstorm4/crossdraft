...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if stage == "build":
            ...
            svg = os.path.join(HERE, "build", cipher, "trail.svg")
        ...
        body = {"ok": ok, "log": log}
        if stage == "analyse":
            path = os.path.join(HERE, "build", cipher, "trail.svg")
...
def main():
    ...
    print(f"digbridge GUI on http://127.0.0.1:{port}")
