...
def cluster(cipher, input_diff, output_diff, weight, extra_weight=0,
    ...
    if verbose:
        # Pad to the block width. Printed with %#x a difference whose top
        # nibbles are zero comes out short, and a 128-bit value shown as
        # eighteen digits looks like a different quantity entirely.
        w = block_bits // 4
        print(f"  differential 0x{input_diff:0{w}x} -> 0x{output_diff:0{w}x}")
