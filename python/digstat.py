...
def _rk_verdict(gap, single_weight, rounds=None):
    """
    What the gap does and does not say.

    Deliberately cautious about a single round count.  A gap at three rounds
    says the two models differ there; whether it persists is the question,
    and only a sweep answers it.  Reporting one round count as though it
    settled the matter is how a distinguisher gets overclaimed.
    """
    where = f" at {rounds} rounds" if rounds else ""
    if gap <= 0:
        return (f"no advantage from relating the keys{where}; the key "
                f"schedule is contributing as much as the data path")
    frac = gap / single_weight if single_weight else 1.0
    scale = ("over half the single-key weight" if frac >= 0.5
             else f"{frac:.0%} of the single-key weight")
    return (f"related-key characteristics are {gap:g} bits cheaper{where}, "
            f"{scale}. One round count settles nothing on its own -- what "
            f"matters is whether the gap widens with rounds or the two "
            f"converge, so sweep several before concluding anything. If it "
            f"does widen, the deployment question follows: can an attacker "
            f"relate keys at all?")


def key_schedule_report(keystep_netlist, spec, rounds=4, samples=400,
                        seed=0):
    """
    Is the KEY SCHEDULE linear over GF(2)?

    Tests the schedule circuit itself, iterated, rather than the whole
    cipher.  Those are different questions and only the first one bears on
    related-key cost: LLBC's round function XORs a round key into L and then
    sends it through an S-box, so the cipher is non-linear in the key however
    linear the schedule is.  Measuring the cipher and reporting it as a
    property of the schedule -- as an earlier version of this function did --
    answers the wrong question and answers it confidently.

    Linearity means K(a) xor K(b) equals K(a xor b) xor K(0) for every pair,
    so random pairs settle it: one counterexample proves non-linearity, and
    none over a few hundred pairs is strong evidence, since a single
    non-linear component would show up almost at once.

    It matters because a linear schedule carries a master-key difference to
    every round key with probability 1.  The difference then costs nothing on
    the key side, and whatever related-key advantage exists comes for free.
    """
    import random

    from dig2claasp import RoundSpec, iterate

    ks = RoundSpec(keystep_netlist)
    key_bits = spec["key_bits"]
    random.seed(seed)

    def derive(master):
        """Round keys after `rounds` steps, as one integer."""
        st, off = {}, 0
        wide = all(re.search(r"_\d+$", n) for n in ks.state)
        if not wide:
            off = key_bits
        for name in ks.state:
            w = ks.bits[name]
            if wide:
                st[name] = (master >> off) & ((1 << w) - 1)
                off += w
            else:
                off -= w
                st[name] = (master >> off) & ((1 << w) - 1)
        for i in range(rounds):
            kp = spec.get("key_params", lambda i, k: {})(i, master)
            st = iterate(keystep_netlist, 1, st,
                         params=lambda _, kp=kp: kp, spec=ks)
        acc = 0
        for n in ks.state:
            acc = (acc << ks.bits[n]) | st[n]
        return acc
    ...
    for _ in range(samples):
        ...
        if derive(a) ^ derive(b) != derive(a ^ b) ^ zero:
            counterexample = (a, b)
            break

    linear = counterexample is None
    return {
        "rounds": rounds,
        "samples": samples,
        "linear": linear,
        ...
        "note": (
            f"the key schedule is affine over GF(2) after {rounds} steps. A "
            f"master-key difference therefore reaches every round key with "
            f"probability 1, and costs nothing on the key side"
            if linear else
            f"the key schedule is not affine after {rounds} steps; a key "
            f"difference does not propagate for free"),
        "caveat": ("non-linearity found by counterexample is proof; "
                   "linearity here is evidence over "
                   f"{samples} random pairs, not proof"),
    }
